...
float normaliser_vecteur(float *vecteur, int taille_vecteur)
{
	...
	#pragma omp parallel for private(i) /*reduction(+:somme_des_carres)*/ schedule(dynamic,1)
...
float multiplication_vectoriel(float *vecteurUn, float *vecteurDeux, int taille_vecteur)
{
	...
	#pragma omp parallel for private(i) /*reduction(+:somme)*/ schedule(dynamic,1)
	for(i=0; i<taille_vecteur; i++)
	{
...
void methode_puissances(float **matrice_A, int nbr_colonnes_A, int nbr_lignes_A, float *vecteur_initial, int taille_vecteur_initial, float tolerance, int nbr_maxi_iteration_possibles)
{
	...
	float *vecteur_propore = (float*) malloc(sizeof(float)*taille_vecteur_initial);
	...
	for(i=0; i < nbr_maxi_iteration_possibles; i++)
	{
		...
		if(res > tolerance)
		{
			...
			#pragma omp parallel for private(i) shared(vecteur_propore,vecteur_q,vecteur_z,vecteur_normalise) schedule(dynamic,1)
			for(i=0; i<taille_vecteur_initial; i++)
			{
				vecteur_q[i] = vecteur_z[i] / vecteur_normalise ;
				vecteur_propore[i] = vecteur_q[i];
			}
			...
			c = multiplication_vectoriel(vecteur_y1, vecteur_propore, taille_vecteur_initial);
...
int main(void)
{
	...
	double debutChrono = omp_get_wtime();
	...
	double finChrono = omp_get_wtime();
	double duree = finChrono - debutChrono;
	printf("temps : %3f\n", duree);
